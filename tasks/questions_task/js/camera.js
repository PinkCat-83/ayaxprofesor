...
import { state, $, worldGroup, rockGroups, getNextRockSets } from './state-dom.js';
...
// ── LAYOUT DEL MUNDO ──────────────────────────────────────
export function resetWorldLayout({ skipRocks = false } = {}) {
...
// Posiciona y etiqueta el set inactivo en las coordenadas de la siguiente ronda,
// antes de que empiece el scroll. Recibe las opciones ya barajadas.
export function prepararSiguienteRonda(nextOffset, opcionesBarajadas) {
  const { groups, labels } = getNextRockSets();
  groups.forEach((g, i) => {
    g.setAttribute('transform', `translate(${nextOffset + ROCK_CX_LOCAL[i]},${ROCK_CY})`);
    g.classList.remove('correct', 'incorrect', 'rock-shaking', 'rock-sinking');
    g.style.visibility = '';
    g.removeAttribute('opacity');
    g.querySelectorAll('.rock-crack').forEach(el => el.remove());
    labels[i].textContent = opcionesBarajadas[i];
  });
}
